...
def analyze_banana_labels(event=None, context=None):
    ...
    try:
        ...
        # Modificar la consulta para usar un formato de fecha más simple
        query = f"""
        ...
        print("Ejecutando consulta para las últimas 24 horas...")
        query_job = bq_client.query(query)
        rows = list(query_job.result())
        
        if not rows:
            print("No se encontraron registros para analizar en las últimas 24 horas")
            # Consulta alternativa sin filtro de fecha para verificar si hay datos
            query = f"""
                SELECT Id, Titulo, Comentario
                FROM `{PROJECT_ID}.{BQ_DATASET}.{BQ_INPUT_TABLE}`
                LIMIT 5
            """
            print("\nVerificando si hay datos en la tabla (sin filtro de fecha)...")
            query_job = bq_client.query(query)
            rows = list(query_job.result())
            if not rows:
